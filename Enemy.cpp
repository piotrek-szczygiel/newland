...
void Enemy::AiUpdate(float frameTime, Location* location, std::vector<sf::FloatRect>* playerCollisions, sf::Vector2f playerPosition)
{
	if(dead)
	{
	...
	if(enemyAggroRange.contains(playerFoots) && CheckPath(sf::FloatRect(playerPosition.x+14,playerPosition.y+32,4,4)))
	{
	...
	if(enemyState == Walking)
	{
		...
		if((npcCollision.left < 0) || (npcCollision.top < 0) || (npcCollision.left+npcCollision.width > location->GetSize().x) || (npcCollision.top+npcCollision.height > location->GetSize().y))
		{
			collision = true;
		}
...
{
	if((startSwordSwing + sf::seconds(0.2f)) < timer.getElapsedTime() && (lastSwordSwing + sf::seconds(1.5f)) < timer.getElapsedTime())
	{
...
void Enemy::DrawNick(sf::RenderWindow& rt, sf::RenderStates states)
{
	...
	enemyName.setPosition(centerPosition.x - (enemyName.getGlobalBounds().width/2), centerPosition.y-(enemyName.getGlobalBounds().height+7));
	enemyNameBg.setPosition(centerPosition.x - (enemyNameBg.getGlobalBounds().width/2)+1, centerPosition.y-(enemyNameBg.getGlobalBounds().height+1));

	enemyHp.setPosition(centerPosition.x - (enemyHp.getGlobalBounds().width/2), centerPosition.y-(enemyHp.getGlobalBounds().height+23));
	enemyHpBg.setPosition(centerPosition.x - (enemyHpBg.getGlobalBounds().width/2)+1, centerPosition.y-(enemyHpBg.getGlobalBounds().height+17));
