...
void UserInterface::Draw(std::string sTime, sf::RenderWindow& rt)
{
	...
	if(showInventory)
	{
		...
		{			
			if(itemsTextures[i].loadFromFile("Resources/items/"+Inventory::GetInventory()->at(i).itemName+".png"))
			{
		...
		while(1)
		{
			...
			sf::Vector2f inventoryPosition(Camera::cameraPosition.x + ((Camera::screenDimensions.x - inventorySprite.getGlobalBounds().width)/2), Camera::cameraPosition.y + 100 + ((Camera::screenDimensions.y - inventorySprite.getGlobalBounds().height)/2));

			inventorySprite.setPosition(inventoryPosition);
			itemInfoSprite.setPosition(Camera::cameraPosition.x + ((Camera::screenDimensions.x - itemInfoSprite.getGlobalBounds().width)/2), inventoryPosition.y - 200);
			description.setPosition(itemInfoSprite.getPosition().x+10.f, itemInfoSprite.getPosition().y+40.f);
			
			moneySprite.setPosition(Camera::cameraPosition.x + Camera::screenDimensions.x - moneySprite.getGlobalBounds().width, Camera::cameraPosition.y + Camera::screenDimensions.y - moneySprite.getGlobalBounds().height - 50.f);
			...
			for(int i = 0; i < nItems; i++)
			{
				if(i>11)
				{
					itemsSprites[i].setPosition(inventoryPosition.x+((i-12)*54)+20, 166+(inventoryPosition.y));
				}
				else if(i>5)
				{
				...
				{
					equippedSprites[i].setPosition(itemsSprites[i].getPosition().x - 6, itemsSprites[i].getPosition().y - 6);
					rt.draw(equippedSprites[i]);
				}
			}

			selectedItemSprite.setPosition(itemsSprites[selectPosition].getPosition().x - 8, itemsSprites[selectPosition].getPosition().y - 8);
			
			if(nItems != 0 )rt.draw(selectedItemSprite);

			if(nItems != 0)
				description.setString(Inventory::GetDescription(Inventory::GetInventory()->at(selectPosition).itemName));
	...
	if(Inventory::GetItemInfo("zegarek").count > 0)
	{
		...
		timeText.setPosition(Camera::cameraPosition.x+GameEngine::screenDimensions.x-150, Camera::cameraPosition.y+GameEngine::screenDimensions.y-20-timeText.getGlobalBounds().height);
...
void UserInterface::DrawMessage(sf::RenderWindow& rt)
{
	if(messageType == NORMAL_MESSAGE)
	{
		...
		messageSprite.setPosition(((GameEngine::screenDimensions.x - messageSprite.getGlobalBounds().width)/2) + position.x, GameEngine::screenDimensions.y - messageSprite.getGlobalBounds().height + position.y - 3);
...
void UserInterface::Popup(std::wstring wstr, sf::Vector2f centerPosition, float hideTime, sf::Color color, bool eraseLast)
{
	sf::Text text(wstr, normalFont, (unsigned int)GameEngine::screenDimensions.y/40);
	text.setColor(color);
	text.setCharacterSize(15);
	text.setPosition(centerPosition.x - (text.getGlobalBounds().width/2), centerPosition.y-(32+text.getGlobalBounds().height));
	...
	else if((text.getPosition().x + text.getGlobalBounds().width) > (Camera::cameraPosition.x + GameEngine::screenDimensions.x))
		text.setPosition(Camera::cameraPosition.x + GameEngine::screenDimensions.x - text.getGlobalBounds().width, text.getPosition().y);
...
void UserInterface::InteractionPopup(std::wstring wstr, sf::Vector2f centerPosition, int button, float hideTime, sf::Color color, bool eraseLast)
{
	sf::Text text(wstr, normalFont, (unsigned int)GameEngine::screenDimensions.y/40);
	text.setColor(color);
	text.setCharacterSize(15);
...
void UserInterface::DrawPopups(sf::RenderWindow& rt)
{
	...
	for(int i = 0; i < interactionPopups.size(); i++)
	{
		...
		button.setPosition(interactionPopups[i].text.getPosition().x - 35, interactionPopups[i].text.getPosition().y-7);
		...
		if(interactionPopups[i].difference != 0.0f)
		{
			...
			color.a = (sf::Uint8)(((interactionPopups[i].hideTime - runTime)/interactionPopups[i].difference)*255.0f);
	...
	for(int i = 0; i < playerPopups.size(); i++)
	{
		if(playerPopups[i].popupInfo.hideTime <= runTime)
		{
			...
			{
				sf::Vector2f centerPosition (playerPopups[i].player->GetPosition().x+16, playerPopups[i].player->GetPosition().y-32);
				playerPopups[i].popupInfo.text.setPosition(centerPosition.x - (playerPopups[i].popupInfo.text.getGlobalBounds().width/2), centerPosition.y-playerPopups[i].popupInfo.text.getGlobalBounds().height);

				if(playerPopups[i].popupInfo.text.getPosition().x < Camera::cameraPosition.x)
					playerPopups[i].popupInfo.text.setPosition(Camera::cameraPosition.x+4, playerPopups[i].popupInfo.text.getPosition().y);
				else if((playerPopups[i].popupInfo.text.getPosition().x + playerPopups[i].popupInfo.text.getGlobalBounds().width) > (Camera::cameraPosition.x + GameEngine::screenDimensions.x))
					playerPopups[i].popupInfo.text.setPosition(Camera::cameraPosition.x + GameEngine::screenDimensions.x - playerPopups[i].popupInfo.text.getGlobalBounds().width-4, playerPopups[i].popupInfo.text.getPosition().y);


				playerPopups[i].popupInfo.bg.setSize(sf::Vector2f(playerPopups[i].popupInfo.text.getGlobalBounds().width+6, playerPopups[i].popupInfo.text.getGlobalBounds().height+6));
				playerPopups[i].popupInfo.bg.setPosition(playerPopups[i].popupInfo.text.getGlobalBounds().left-3, playerPopups[i].popupInfo.text.getGlobalBounds().top-3);
		...
		if(playerPopups[i].popupInfo.difference!=0.0f)
		{
			...
			color.a = (sf::Uint8)(((playerPopups[i].popupInfo.hideTime - runTime)/playerPopups[i].popupInfo.difference)*255.0f);
