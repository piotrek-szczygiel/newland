...
{
	if((playerCollision.left < 0) || (playerCollision.top < 0) || (playerCollision.left+playerCollision.width > currentLocation->GetSize().x) || (playerCollision.top+playerCollision.height > currentLocation->GetSize().y))
		return true;
	return false;
}
