...
void GameEngine::Initialize(float x, float y, std::string title, bool _fullscreen, std::string arg)
{
	...
	renderWindow = new sf::RenderWindow(sf::VideoMode((unsigned int)screenDimensions.x,(unsigned int)screenDimensions.y), title, _fullscreen ? sf::Style::Fullscreen : sf::Style::Close);
	//renderWindow->setVerticalSyncEnabled(true);
	// Utworzenie obiektu walsciwej gry
	if(_fullscreen)renderWindow->setMouseCursorVisible(false);
...
int GameEngine::Run()
{
	...
	while(renderWindow -> isOpen() && Game::running)
	{
		...
		if(!paused)
		{
			...
			fpsText.setPosition(Camera::cameraPosition.x+screenDimensions.x-fpsText.getGlobalBounds().width-5, Camera::cameraPosition.y);
...
void GameEngine::MainMenu()
{
	...
	while(renderWindow->isOpen())
	{
		...
		if(actualFrame + 1 == maxFrames) actualFrame = 0;

		for(int i = 0; i <= max_position; i++)
		{
			if(i == position)
			{
				pozycje_active[i]->setPosition(GameEngine::screenDimensions.x / 2 - (pozycje_active[i]->getGlobalBounds().width / 2), 240 + i * 60);
				renderWindow->draw(*pozycje_active[i]);
			}
			else
			{
				pozycje[i]->setPosition(GameEngine::screenDimensions.x / 2 - (pozycje_active[i]->getGlobalBounds().width / 2), 240 + i * 60);
