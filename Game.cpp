...
void Game::Update(sf::Time _frameTime)
{
	...
	ssTime << "Godzina: " << std::setfill('0') << std::setw(2) << floorf(((fmod(dayTimer + 17.5, 60.f)/2.5f)*3600.f)/3600.f) \
		           << ":" << std::setfill('0') << std::setw(2) << floorf(fmod(((fmod(dayTimer + 17.5, 60.f)/2.5f)*3600.f)/60.f, 60.f)); // obliczanie minut na podstawie timera
	...
	// Test kolizji
	if((Maps::MapCollision(player.GetCollisionPoints()) || Maps::currentLocation->Collision(player.GetCollisionPoints()) || Maps::currentLocation->NpcCollision(player.GetCollisionPoints())))
	{
...
void Game::Draw(sf::RenderWindow& rt, sf::RenderStates states)
{
	...
	Maps::currentLocation->Draw(rt, states, &player, GameEngine::runtimeClock.getElapsedTime().asSeconds(), frameTime.asSeconds(), daynightType);
