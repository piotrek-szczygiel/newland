...
void Gameplay::Interaction()
{
	...
	for(int i = 0; i < triggers->size(); i++)
		// Jezeli gracz stoi na obiekcie wywolujacym - przekazujemy dzialanie funkcji Trigger z parametrem danego obiektu
	...
	if(npcs != NULL)
		...
		for(int i = 0; i < Maps::currentLocation->pickups.size(); i++)
		{
			...
			{
				UserInterface::InteractionPopup(L"Podnieś przedmiot", sf::Vector2f(Maps::currentLocation->pickups[i]->itemSprite.getPosition().x+16, Maps::currentLocation->pickups[i]->itemSprite.getPosition().y+16), UserInterface::Kwadrat, 0.5f, sf::Color::Yellow, true);
...
void Gameplay::Trigger(std::wstring objName)
{
	...
	if(npc!=NULL)npc->interaction = true;
	else return;
	...
	if(location == "farma")
	{
		...
		else if(objName == L"Handlarz")
		{
			...
			if(Controls::IsPressed(Controls::Choose1))
			{
				...
				{
					UserInterface::Popup(L"Niewystarczająca ilość pieniędzy!", &Game::player, 3.0f, sf::Color::Red, true);
				}
			}
...
void Gameplay::Pustynia(std::wstring objName, std::string location, sf::Vector2f objCenterPos)
{
	...
	if(location == "pustynia")
	{
		...
		else if(objName == L"skrzynia")
		{
			...
			{
				UserInterface::InteractionPopup(L"Weź przedmioty ze skrzynki", objCenterPos, UserInterface::Kwadrat, 0.5f, sf::Color::Yellow, true);
				if(Controls::IsPressed(Controls::Pickup))
				{
		...
		else if(objName == L"skrzynia_roza")
		{
			if(GameplayInfo::GetInfo("roza", "quest").count == 1)
			{
				...
				{
					UserInterface::InteractionPopup(L"Zabierz skarby", objCenterPos, UserInterface::Kwadrat, 0.5f, sf::Color::Yellow, true);
					if(Controls::IsPressed(Controls::Pickup))
					{
		...
		else if(objName == L"zaklecie")
		{
			if(!GameplayInfo::GetInfo("grota", "otworzona").count && Inventory::GetItemInfo("zaklecie").count > 0)
			{
				...
				{
					if(Maps::GetLocation("pustynia")->GetObjects()->at(i).GetName()=="grota1" || Maps::GetLocation("pustynia")->GetObjects()->at(i).GetName()=="grota2")
					{
		...
		else if(objName == L"Wygnaniec")
		{
			...
			if(GameplayInfo::GetInfo("wygnaniec", "quest").count == 0)
			{
				...
				if(rozmowa == 0)
				{
					UserInterface::MessageWait(L"Witaj podróżniku jestem ciężko chory i potrzebuję Twojej Pomocy!\n\n[1] - Co mam zrobić?       [2] - Nie mam na to czasu!", npc, 0.f);
					if(Controls::IsPressed(Controls::Choose1))
					{
				...
				{
					UserInterface::MessageWait(L"Podróżniku proszę nie zapomnij o mnie i wróć kiedy znajdziesz czas.\nPozostało mi niewiele czasu...", npc, 0.f);
				}

				//co mam zrobic
				else if(rozmowa == 2)
				{
					UserInterface::MessageWait(L"Kilka lat temu za drobne przewinienie wyrzucono mnie z miasta\ni zostałem skazany na banicję. Czy możesz zabrać tę butelkę\ni napełnić ją wodą z miejscowej studni? Już od kilku dni nic nie piłem.\n\n[1] - Dlaczego Cię wygnano?\n[2] - Chętnie Ci pomogę!\n[3] - Nie mam na to czasu!", npc, 0.f);
					if(Controls::IsPressed(Controls::Choose1))
					{
				...
				{
					UserInterface::MessageWait(L"A więc, jeżeli już musisz wiedzieć kilka lat temu, gdy ta kraina była jedną\nz najspokojniejszych na całym świecie a mieszkańcy New Land nie\nmartwili się potworami, pracowałem w gospodzie. Pewnej nocy gdy\ndozorca jeszcze spał zakradłem się do jego piwnicy i zjadłem odrobinę\nprzysmaków jakie zostały sprowadzone z innych krain.\n\n[ENTER] - Dalej", npc, 0.f);
				...
				{
					UserInterface::MessageWait(L"Nie spodziewałem się że mój spisek wyjdzie na jaw i co gorsza\nże zostanę oskarżony o przywoływanie duchów. Jak później się\ndowiedziałem gospodarz przechowywał zakazane relikwie dla\njakieś znachorki i całą winę zrzucił na mnie.\n\n[1] - To naprawdę niesprawiedliwe oczywiście że ci pomogę!\n[2] - Nic z tego nie będe pomagać przestępcy!", npc, 0.f);
					if(Controls::IsPressed(Controls::Choose1))
					{
						GameplayInfo::SetInfo("wygnaniec", "rozmowa", 6);
					}
				...
				{
					UserInterface::MessageWait(L"Nie sądziłem że nie będziesz taki niewyrozumiały.\nNaprawdę na ciebie liczyłem...", npc, 0.f);
				}
				else if(rozmowa == 6)
				{
					UserInterface::MessageWait(L"Dobrze a więc weź tą butelkę i napełnij ją wodą w miejscowej studni.\nWprawdzie nie jestem już teraz zbyt bogaty,\nale postaram się odwdzięczyć.\n\n[ENTER] - Dobrze, ide napełnić butelke!", npc, 0.f);
					if(Controls::IsPressed(Controls::Accept))
					{
	...
	else if(location == "lochy")
	{
		...
		else if(objName == L"grobowiec")
		{
			if(GameplayInfo::GetInfo("znachorka", "quest").count == 1)
			{
				...
				{
					UserInterface::InteractionPopup(L"Zabierz kości", objCenterPos, UserInterface::Kwadrat, 0.5f, sf::Color::Yellow, true);
					if(Controls::IsPressed(Controls::Pickup))
					{
...
void Gameplay::Plaza(std::wstring objName, std::string location, sf::Vector2f objCenterPos)
{
	...
	if(location == "plaza")
	{
		...
		else if(objName == L"sterowanie")
		{
			...
			{
				UserInterface::Message(L"Witaj! Czy chcesz zobaczyć konfigurację sterowania?\n\n[ENTER] - Tak\n[ESCAPE] - Pomiń");
				if(Controls::IsPressed(Controls::Accept))
				{
...
void Gameplay::Galicja(std::wstring objName, std::string location, sf::Vector2f objCenterPos)
{
	...
	if(location == "galicja")
	{
		...
		else if(objName == L"fontanna")
		{
			...
			{
					UserInterface::InteractionPopup(L"Napełnij butelke", objCenterPos, UserInterface::Kwadrat, 0.5f, sf::Color::Yellow, true);
					if(Controls::IsPressed(Controls::Pickup))
					{
		...
		{
			UserInterface::MessageWait(L"Witaj, mogę Ciebie całkowicie uleczyć za drobną opłatą...\n\n[1] - Skorzystaj z usługi (50$)", npc, 0.f);
			if(Controls::IsPressed(Controls::Choose1))
			{
		...
		else if(objName == L"Znachorka")
		{
			if(GameplayInfo::GetInfo("znachorka", "quest").count == 0)
			{
				if(GameplayInfo::GetInfo("grota", "otworzona").count == 1)
				{
					...
					{
						UserInterface::MessageWait(L"Och niewdzięczniku! Gdzie są prawdziwi bohaterowie\nna tym świecie!\n\nMam nadzieję że jeszcze zmienisz zdanie!", npc, 0.f);
					}

					//tak o co chodzi
					else if(rozmowa == 2)
					{
						UserInterface::MessageWait(L"Wyglądasz mi na silnego wojownika.\nPotrzebuję właśnie kogoś takiego jak ty.\nJestem znachorką, zajmuję się leczeniem ludzi, ale\nw dzisiejszych niegodziwych czasach ludzie oczekują\node mnie więcej… Potrzebuję kośći zmarłego.\n\n[1] - Po co ci to cholerstwo?\n[2] - Gdzie mogę coś takiego znaleźć?", npc, 0.f);
						if(Controls::IsPressed(Controls::Choose1))
						{
					...
					{
						UserInterface::MessageWait(L"Jeśli już musisz wiedzieć… Do stworzenia mikstury regeneracji.\nBez niej moi słudzy umrą\n\n[1] - Słudzy? Co masz na myśli?\n[2] - Dobra biore się za to!", npc, 0.f);
						if(Controls::IsPressed(Controls::Choose1))
						{
					...
					{
						UserInterface::MessageWait(L"W opuszczonej jaskini na pustyni znajduje się wejście do groty.\nTam szukaj kości przy grobowcu.\n\n [1] - A więc ide przeszukać jaskinię!", npc, 0.f);
						if(Controls::IsPressed(Controls::Choose1))
						{
					...
					{
						UserInterface::MessageWait(L"Nie twoja sprawa. Pomożesz mi czy nie?\n\n[1] - Nie, obawiam się że w tej sytuacji nie mogę ci pomóc.\n[2] - Dobra biorę się za to!", npc, 0.f);
						if(Controls::IsPressed(Controls::Choose1))
						{
							GameplayInfo::SetInfo("znachorka", "rozmowa", -1);
						}
				...
				{
					UserInterface::Popup(L"Kiedy wreszcie ktoś odnajdzie wejście\ndo tej przeklętej groty!", &npc->player, 4.f);
				}
			}
			else if(GameplayInfo::GetInfo("znachorka", "quest").count == 1)
			{
				...
				{
					UserInterface::MessageWait(L"Wiedziałam że mogę na tobie polegać!\nW nagrode dostaniesz troche monet i coś co może\nci się przydać!\n\n[ENTER] - Akceptuj", npc, 0.f);
					if(Controls::IsPressed(Controls::Accept))
					{
						Inventory::money += 50;
				...
				{
					if(lastObject != L"Znachorka")
						UserInterface::Popup(L"Wroć kiedy znajdziesz kości!\nPotrzebuje szybko zrobić miksture!", &npc->player, 3.0f);
				}
			}
		...
		else if(objName == L"Strażnik")
		{
			if(GameplayInfo::GetInfo("beczki", "przesuniete").count==0)
			{
				...
				if(rozmowa == 1)
				{
					if(Inventory::GetItemInfo("przepustka").count > 0)
					{
							...
							{
								if(Maps::currentLocation->GetObjects()->at(i).GetName()=="beczka1" || Maps::currentLocation->GetObjects()->at(i).GetName()=="beczka2")
								{
			...
			{
				if(lastObject != L"Strażnik")
					UserInterface::Popup(L"Nie zawracaj mi głowy, pilnuje przejścia.", &npc->player, 1.5f, sf::Color::White, true);
			}
		}
	}
	else if(location == "karczma")
	{
		...
		else if(objName == L"Róża")
		{
			if(GameplayInfo::GetInfo("roza", "quest").count == 0)
			{
				...
				{
					UserInterface::MessageWait(L"A więc kilka tygodni mój mąż udał się na pustynie w poszukiwaniu\nskarbów, lecz nigdy nie wrócił. Proszę weź ten klucz, przed jego\nśmiercią umówiliśmy się że ukryje tam wszystkie skarby, wśród nich\npowinny być także leki, których dla mnie szukał. Proszę weź wszystko\ndla siebie i wróc tutaj a odkupię od ciebie te lekarstwa. Bardzo mi na\ntym żależy. Skarby nie są dla mnie tak ważne...\n\n[1] - Jasne że ci pomogę!      [2] - Przykro mi, ale nie mam czasu.", npc, 0.f);
			...
			else if(GameplayInfo::GetInfo("roza", "quest").count == 1)
			{
				...
				{
					UserInterface::MessageWait(L"Dziękuje!\nTo dzięki takim jak ty New Land jest\ntak wspaniałą krainą. Bardzo mi pomogłeś.", npc, 5.f);
	...
	else if(location == "domek1")
	{
		...
		else if(objName == L"Zabójca")
		{
			if(GameplayInfo::GetInfo("zabojca", "quest").count == 0)
			{
				...
				if(rozmowa == 0)
				{
					UserInterface::MessageWait(L"Witaj! Potrzebuje czaszek szkieletów do uzupełnienia mojej wystawy...\nW zamian otrzymasz nagrodę...\n\n[1] - Okej, mogę spróbować!     [2] - Nie mam czasu na takie zabawy.", npc, 0.f);
					if(Controls::IsPressed(Controls::Choose1))
					{
				...
				{
					UserInterface::MessageWait(L"Dobra słuchaj... Masz tu zaklęcie które wypowiesz stojąc między dwoma\nfilarami ze słońcem i księżycem. Znajdują się one obok portu na pustyni.\nPowinna ci się ukazać grota do której wejdziesz.\nW środku znajdziesz szkielety z których możesz otrzymać czaszki.\nPotrzebuje 5 takich czaszek\n[1] - Dobra, rozumiem, biorę się do roboty\n[2] - To jednak nie dla mnie", npc, 0.f);
					if(Controls::IsPressed(Controls::Choose1))
					{
			...
			else if(GameplayInfo::GetInfo("zabojca", "quest").count == 1)
			{
				...
				{
					UserInterface::MessageWait(L"Okej dzięki wielkie. Już ide na piętro, żeby je udekorować.\nMasz tu klucz do skrzynki w mojej sypialni...\nMoże coś ci się sprzyda.\n[ENTER] - Akceptuj", npc, 0.f);
					if(Controls::IsPressed(Controls::Accept))
					{
		...
		{
			if(GameplayInfo::GetInfo("skrzynia_zabojca", "zabrano").count == 0 && Inventory::GetItemInfo("klucz_zabojca").count > 0)
			{
				UserInterface::InteractionPopup(L"Weź przedmioty ze skrzynki", objCenterPos, UserInterface::Kwadrat, 0.5f, sf::Color::Yellow, true);
				if(Controls::IsPressed(Controls::Pickup))
				{
	...
	else if(location == "domek1_piwnica")
	{
		...
		{
			UserInterface::InteractionPopup(L"Zagraj na pianinie", objCenterPos, UserInterface::Trojkat, 0.5f, sf::Color::Yellow, true);
			if(Controls::IsPressed(Controls::Interaction))
			{
				Sound::pianino.play();
