		...
		if(timer.isRunning())
		{
			if(!interaction)
			{
				...
				if(changeDirection)
				{
					...
					if(!free)changeTime = sf::seconds((rand()%(3*longer)) + 0.25) + timer.getElapsedTime();
					else changeTime = sf::seconds((rand()%(5*longer)) + 0.25) + timer.getElapsedTime();
				...
				sf::FloatRect npcCollision=sf::FloatRect(player.GetPosition().x+8,player.GetPosition().y+32,16,16);
