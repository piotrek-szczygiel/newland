...
void Inventory::Initialize()
{
	...
	descriptions->push_back(Descriptions("stara_notatka", L"Notatka znaleziona w starej dziupli.\nNie można się z niej odczytać."));
	descriptions->push_back(Descriptions("przepustka", L"Przepustka zakupiona u handlarza.\nUpoważnia kupców do przejścia przez most."));
	descriptions->push_back(Descriptions("zegarek", L"Najnowszy wynalazek lokalnego starca.\nUmożliwia odczytanie dokładnej pory dnia."));
	descriptions->push_back(Descriptions("patyk", L"Patyk znaleziony na ziemi.\nPodstawowe narzędzie walki zadające 5 obrażeń.\n\nWalczysz za pomocą przycisku [SPACJA]\nMożesz go ubrać naciskając [ENTER]"));
	descriptions->push_back(Descriptions("miecz", L"Stalowy miecz.\nZadaje 15 obrażeń.\n\nWalczysz za pomocą przycisku [SPACJA]\nMożesz go ubrać naciskając [ENTER]"));
	descriptions->push_back(Descriptions("butelka", L"Butelka z listem w środku.\nZawiera starą notatkę."));
	descriptions->push_back(Descriptions("kosci", L"Kosci wydobyte z grobowca w lochach.\nZnachorka potrzebuje ich do zrobienia mikstur."));
	...
	descriptions->push_back(Descriptions("hp_pot", L"Mikstura odnawiająca 30 zdrowia.\nMożesz jej użyć naciskając [ENTER]"));
	...
	descriptions->push_back(Descriptions("zaklecie", L"Stare zaklęcie. Prawdopodobnie otwiera\nswego rodzaju przejście."));
}
...
void Inventory::DeleteItem(std::string itemName, int count)
{
	for(int i = 0; i < inventory->size(); i++)
	{
		if(inventory->at(i).itemName == itemName)
		{
			...
			if(inventory->at(i).count <= 0 || count == 0) //jezeli usunieto tyle przedmiotow ze juz nie ma wcale itemu, usuwamy go z tablicy
			{
