...
{
	if((startSwordSwing + sf::seconds(0.2f)) < timer.getElapsedTime() && (lastSwordSwing + sf::seconds(0.35f)) < timer.getElapsedTime())
	{
...
void Player::DrawNick(sf::RenderWindow& rt, sf::RenderStates states, bool interaction)
{
	if(!mainPlayer)
	{
		...
		playerName.setPosition(centerPosition.x - (playerName.getGlobalBounds().width/2), centerPosition.y-(playerName.getGlobalBounds().height+7));
		playerNameBg.setPosition(centerPosition.x - (playerNameBg.getGlobalBounds().width/2)+1, centerPosition.y-(playerNameBg.getGlobalBounds().height+1));
		interactionIconSprite.setPosition(playerNameBg.getPosition().x-20, playerNameBg.getPosition().y);

		if(interaction)rt.draw(interactionIconSprite);
	...
	else
	{
		...
		//playerNameBg.setSize(sf::Vector2f(playerName.getGlobalBounds().width+4, playerName.getGlobalBounds().height+4));
		//playerName.setPosition(centerPosition.x - ((playerName.getGlobalBounds().width/2)+(playerHp.getGlobalBounds().width/2)), centerPosition.y-(playerName.getGlobalBounds().height+7));
		//playerNameBg.setPosition(centerPosition.x - ((playerNameBg.getGlobalBounds().width/2)+(playerHpBg.getGlobalBounds().width/2))+1, centerPosition.y-(playerNameBg.getGlobalBounds().height+1));
