...
{
	cameraPosition = sf::Vector2f(playerPosition.x + 16 - (screenDimensions.x / 2), playerPosition.y + 16 - (screenDimensions.y / 2));
		
		
		if(mapDimensions.x > screenDimensions.x)
		{
			if(cameraPosition.x < 0) // JESLI KAMERA MIALA BY POKAZAC POZA GRANICA MAPY TO USTAWIAMY KAMERE TAK ZEBY GRACZ NIE BYL W SRODKU - KAMERA BEDZIE POKAZYWAC OKRESLONY ROG MAPY
			    cameraPosition.x = 0;
