	...
	for(auto layer = objectLayers.begin(); layer != objectLayers.end(); ++layer)
	{
			...
			for(auto obj = layer->objects->begin(); obj != layer->objects->end(); ++obj)
			{
				for(int i = 0; i < 4; i++)
				{
					...
					case 0:
						...
						break;
					case 1:
						enemy->SetPosition(sf::Vector2f(obj->GetAABB().left + 40, obj->GetAABB().top + (obj->GetAABB().height) - 40));
						break;
					case 2:
						enemy->SetPosition(sf::Vector2f(obj->GetAABB().left + (obj->GetAABB().width) - 40, obj->GetAABB().top + 40));
						break;
					case 3:
						enemy->SetPosition(sf::Vector2f(obj->GetAABB().left + (obj->GetAABB().width) - 40, obj->GetAABB().top + (obj->GetAABB().height) - 40));
						break;
					case 4:
						enemy->SetPosition(sf::Vector2f(obj->GetAABB().left + (obj->GetAABB().width/2), obj->GetAABB().top + (obj->GetAABB().height/2)));
						break;
					default:
						break;
					}
		...
		else if(layer->layerName == "Light")
		{
			for(auto obj = layer->objects->begin(); obj != layer->objects->end(); ++obj)
			{
				...
				float scalex, scaley;
				scalex = scaley = obj->GetPropertyString("scale") == "" ? 1.f : std::atof(obj->GetPropertyString("scale").c_str());
				light.setScale(scalex, scaley);
				light.setPosition(obj->GetCentre().x-(light.getGlobalBounds().width/2), obj->GetCentre().y-(light.getGlobalBounds().height/2));
				
				std::string lightColor = obj->GetPropertyString("color") == "" ? "white" : obj->GetPropertyString("color");

				if(lightColor == "yellow") light.setColor(sf::Color(255,255,128));
				else if(lightColor == "orange") light.setColor(sf::Color(255,128,64));
...
// Rysowanie mapy
void Location::Draw(sf::RenderWindow &rt, sf::RenderStates states, Player* player, float runTime, float frameTime, int daynightType)
{
